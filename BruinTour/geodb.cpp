...
bool GeoDatabase::load(const string& map_data_file)
{
    ...
    {   // already read in streetName
        ...
        GeoToGeo[geopoint1.to_string()].push_back(geopoint2);
        GeoToGeo[geopoint2.to_string()].push_back(geopoint1);
        ...
        if (poiNum > 0)
        {
            ...
            GeoToGeo[geopoint1.to_string()].push_back(geopointMid);
            GeoToGeo[geopointMid.to_string()].push_back(geopoint1);
            ...
            GeoToGeo[geopoint2.to_string()].push_back(geopointMid);
            GeoToGeo[geopointMid.to_string()].push_back(geopoint2);
            ...
            for (int i = poiNum; i > 0; i--)
            {
                ...
                GeoToGeo[geopointPOI.to_string()].push_back(geopointMid);
                GeoToGeo[geopointMid.to_string()].push_back(geopointPOI);
...
vector<GeoPoint> GeoDatabase::get_connected_points(const GeoPoint& pt) const
{
    ...
    vector<GeoPoint> sourceVector(0);
    ...
    {
        connectedPoints.push_back(sourceVector[i]);
    }
